...
const util = {
    ...
    */
        ...
        if(aValid.includes(false)){
            ...
            switch(frm){ 
                ...
                case '#loginForm':
                    ...
                    //--orig -- util.loginPost(frm ,frmModal,`https://osndp.onrender.com/loginpost/${objfrm.uid}/${objfrm.pwd}`)
        ...
        .then((data) => {
            ...
            if(data.found){
                ...
                zonked.socket.on('answered',(data)=>{
                    ...
                    //if they wud accept
                    let ans = confirm(`WOULD YOU LIKE TO ACCEPT THE CALL FROM ${xdata.patient}?`)
                ...
                zonked.socket.on('noconnect',(data)=>{
                    let xdata = JSON.parse(data)
                    alert(`DR. ${xdata.doctor} is not connected...`)
                })
                ...
                zonked.socket.on('confirmed',(data)=>{
                    ...
                    if(xdata.type==="doctor"){
                        ...
                        document.getElementById('connect_to_peer').value= xpeerId
                        //hide the inputbox/button if doctor
                        //document.getElementById('rowvideo').classList.add('lets-hide')
                    }else{
                        document.getElementById('connect_to_peer').value=xdata.doctor+'-'+ xdata.caseno
                        xpeerId = xdata.patient +'-'+ xdata.caseno
                    }   
                   
                    util.scrollsTo('videoshow')
                    zonked.startVideo( xpeerId )               
                    //this will fire playvideofromcam()
                })
                
                //===declined
                zonked.socket.on('declined',(data)=>{
                    let xdata =JSON.parse(data)
                    alert(`DR ${xdata.doctor} is BUSY at the moment and DECLINED, kindly wait for her availability and check again!...`)
                    //return false
                })
    ...
    //====ADD MENU GRPID AFTER LOGIN===//
    addmenu:(grpid)=>{
        const menu = document.getElementById('menulist')

        menu.innerHTML = "" //erase contents

        let aOrigMenu = ["About Us","Services","Contact Us","Login"]
        let aOrigMenuLink = ["#aboutus","#features","#contacts","#login"]
        let origLImenu = ""
        ...
        switch (grpid){
            ...
            case "2": //if grpid 2 doctors 
                ...
                <li class="nav-item dropdown">
                    ...
                    <div class="dropdown-menu" id='drops' >
                        <a style="background-color:red;a.color:white;" class="text-white dropdown-item display-4" href="javascript:zonked.getpatientBooking(${util.grpid})">View Booking</a>
                    </div>
                </li>`
            break
        }
